...
func (s *Service) createResponse(ctx context.Context, modelID uuid.UUID, body []byte) (Response, error) {
	...
	log.Printf("proxy: resolved model remote=%s endpoint=%s", resolved.Model.RemoteName, resolved.Provider.Endpoint)
...
func (s *Service) doRequest(ctx context.Context, resolved ResolvedModel, body []byte) (Response, error) {
	...
	log.Printf("proxy: upstream status=%d", resp.StatusCode)
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("proxy: request method=%s path=%s size=%d", r.Method, r.URL.Path, len(body))
	modelID, stream, err := parseRequestPayload(body)
	if err != nil {
		if rawModel, ok := extractRawModelValue(body); ok {
			log.Printf("proxy: parse payload failed: %v (model=%q)", err, rawModel)
		} else {
			log.Printf("proxy: parse payload failed: %v", err)
		}
		writeProxyError(w, err)
		return
	}
	log.Printf("proxy: parsed payload model_id=%s stream=%t", modelID, stream)
...
func extractRawModelValue(body []byte) (string, bool) {
	if len(body) == 0 {
		return "", false
	}
	var payload map[string]any
...
func writeProxyError(w http.ResponseWriter, err error) {
	...
	log.Printf("proxy: error status=%d err=%v", status, err)
